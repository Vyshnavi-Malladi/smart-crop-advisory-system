...
router.post('/ask', async (req, res) => {
    ...
    if (language === "te") langName = "Telugu";
    ...
    try {
        ...
        const response = await axios.post(`${OLLAMA_URL}/api/generate`, {
            model: "llama3",
            prompt: `You are an expert agricultural assistant. Respond only in ${langName}. Answer the following question for a farmer concisely and helpfully: ${question}`,
            stream: false
        });
